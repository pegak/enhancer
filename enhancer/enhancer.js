...
function formatSettings() {
  ...
  const profileHelp = window.top.$('#profile_help');
...
function hideRecentlyFarmed(cell, profileArray) {
  if (profileArray[s.hide_recent_farms]) {
    ...
    const minutesBetween = Math.abs(parseInt(dif / 1000 / 60));
...
function scoutReportSettings(cell, profileArray) {
  ...
  if (profileArray[s.enable_scout]) {
    if (window.top.$.trim(cell.find('span').html()) == '?') {
      ...
      const wood = parseInt(cell.children('span').eq(0).html().replace(/\D+/g, ''));
      const clay = parseInt(cell.children('span').eq(1).html().replace(/\D+/g, ''));
      const iron = parseInt(cell.children('span').eq(2).html().replace(/\D+/g, ''));
...
function wallSettings(cell, profileArray) {
  if (profileArray[s.enable_walls]) {
    let wallLvl = parseInt(cell.html());
    if (wallLvl == '?') {
      wallLvl = 0;
    }
...
function setDefaultProfile() {
  if (window.top.$('#settingsProfile').val() == profile_10) {
    const newProfile = confirm(dialog_02);
    if (newProfile) {
      createProfile();
      setDefaultProfile();
    } else {
...
function createProfile() {
  const profileName = prompt(`${dialog_03}:`);
...
function exportProfile() {
  ...
  if (profileName == profile_10) {
    ...
    const profileSettings = prompt(dialog_08, `${dialog_09A}${profileName}${dialog_09B}${profileName},${settings}${dialog_09C}`);
  }
}

function importProfile() {
  let profileSettings = prompt(`${dialog_10}:`, dialog_11);
...
function loadLanguage(lang) {
  ...
  const profileList = window.top.$.jStorage.get('profileList');
  const defaultProfile = window.top.$.jStorage.get(`profile:${profile_10}`);
